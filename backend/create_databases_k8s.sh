...
for i in {1..10}; do
  if kubectl exec -it "$POD_NAME" -- psql -U "$POSTGRES_USER" -c "\q"; then
...
# Create databases
kubectl exec -it "$POD_NAME" -- psql -U "$POSTGRES_USER" -c "CREATE DATABASE evaluations;"
kubectl exec -it "$POD_NAME" -- psql -U "$POSTGRES_USER" -c "CREATE DATABASE issues;"

kubectl exec -it "$POD_NAME" -- psql -U "$POSTGRES_USER" -c "\c evaluations" -c "
...
kubectl exec -it "$POD_NAME" -- psql -U "$POSTGRES_USER" -c "\c issues" -c "
...
echo "Databases 'evaluations' and 'issues' created successfully."
